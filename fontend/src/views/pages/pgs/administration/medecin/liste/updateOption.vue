    ...
    <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <template v-if="ajouterAcces">
                            ...
                            <FloatLabel variant="on" v-animateonscroll="{ enterClass: 'animate-enter fade-in-10 zoom-in-50 animate-duration-1000' }">
                                <InputText v-model="login" size="large" class="w-full" :invalid="submitted && ajouterAcces && !login" variant="filled" />
                                <label>Login</label>
                            </FloatLabel>
                            <!-- Mot de passe -->
                            <FloatLabel variant="on" v-animateonscroll="{ enterClass: 'animate-enter fade-in-10 zoom-in-50 animate-duration-1000' }">
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg z-50">
            <div class="flex items-center gap-2">
                <Button icon="pi pi-times" severity="danger" label="Fermer" size="large" class="w-full" @click="drawerUse.hide()" />
                <Button type="submit" form="medecinForm" icon="pi pi-check" severity="success" :loading="loadingForm" :label="loadingForm ? 'Opération en cours...' : 'Mise à jour'" size="large" class="w-full" />
            </div>
        </div>
    </div>
</template>
...
const formSubmit = async () => {
   ...
   try {
      ...
      if (res.status === 200) {
         ...
         props.fetchLists?.()  // 🔥 ici tu appelles la fonction parent
...
watch(
  () => drawerUse.componentRef,
  (val) => {
    if (val) {
      console.log("Composant monté dans le Drawer !")
    }
  }
)
